    ...
    for d in range(30):
        ...
        for dataset in datasets:
            ...
            rows.append({
                ...
                "latest_completed_at": datetime.combine(run_date, datetime.min.time()),
                ...
                "latest_bronze_written_at": datetime.combine(run_date, datetime.min.time()),
...
def data_trust() -> pl.DataFrame:
    ...
    datasets = ["permits", "evictions", "incidents"]
    checks = [
        ("latest_ingest_status", "critical", "Latest ingest interval status is success or empty."),
        ("raw_object_recorded", "critical", "Non-empty extracts have a raw NDJSON object."),
        ("bronze_manifest_recorded", "critical", "Non-empty extracts have a bronze file-manifest event."),
        ("bronze_record_count_matches_extract", "critical", "Bronze records match extracted records."),
        ("metadata_freshness", "warning", "Latest metadata completed within the freshness threshold."),
    ]
    out = []
    for dataset in datasets:
        for check_name, severity, expected_rule in checks:
            ...
            out.append({
                ...
                "latest_bronze_written_at": datetime.combine(today, datetime.min.time()),
