...
<LineChart data={success_trend} x=run_date y=success_rate_pct series=dataset_name yAxisTitle="Success rate %" yMax=100/>

```sql records
select run_date, dataset_name, sum(total_records_ingested) as records_ingested
from sf_urban_health.pipeline_health
group by 1, 2
order by 1, 2
```

<BarChart data={records} x=run_date y=records_ingested series=dataset_name type=stacked/>
...
select dataset_name, check_status, count(*) as checks
from sf_urban_health.data_trust
group by 1, 2
order by 1, 2
```

<BarChart data={trust_by_dataset} x=dataset_name y=checks series=check_status/>
