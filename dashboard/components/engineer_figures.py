...
def dag_success_rate_line(
    ...
    fig.update_layout(
        **tu.base_layout(template),
        ...
        height=280,
        yaxis=dict(title="Success %", range=[-5, 110], gridcolor=tu.grid_color(template)),
...
def test_volume_bar(
    ...
    fig = go.Figure()
    fig.add_bar(
        x=daily["run_date"].to_list(), y=daily["passed"].to_list(),
        name="Passed",
        marker_color=tu.ok_color(template),
        marker_opacity=0.55,
        marker_line_color=tu.ok_color(template), marker_line_width=1,
        hovertemplate="%{x}<br>%{y} passed<extra></extra>",
    )
    fig.add_bar(
        x=daily["run_date"].to_list(), y=daily["failed"].to_list(),
        name="Failed",
        marker_color=tu.bad_color(template),
        marker_opacity=0.55,
        marker_line_color=tu.bad_color(template), marker_line_width=1,
