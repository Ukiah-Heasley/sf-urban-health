...
def incident_trend(df: pl.DataFrame, template: str = "terminal_amber") -> go.Figure:
    ...
    fig = go.Figure(go.Bar(
        x=monthly["incident_month"], y=monthly["total_incidents"],
...
def top_neighborhoods(df: pl.DataFrame, n: int = 15, template: str = "terminal_amber") -> go.Figure:
    ...
    fig = go.Figure(go.Bar(
        x=top["total_incidents"], y=top["neighborhood"],
        orientation="h", marker_color=tu.palette(template)[0],
    ...
    fig.update_layout(
        **tu.base_layout(template, margin=dict(l=180, r=20, t=50, b=40)),
        ...
        height=400, xaxis_title="Incidents", yaxis_title=None,
    )
    return fig


def category_bar(df: pl.DataFrame, n: int = 20, template: str = "terminal_amber") -> go.Figure:
    if df.is_empty():
        return _empty("No data in the selected window", 400, template)

    cats = category_breakdown(df, n)
    fig = go.Figure(go.Bar(
        x=cats["total_incidents"], y=cats["incident_category"],
        orientation="h", marker_color=tu.palette(template)[2],
        hovertemplate="%{y}<br>%{x:,.0f} incidents<extra></extra>",
    ))
    fig.update_layout(
...
def resolution_by_district(df: pl.DataFrame, template: str = "terminal_amber") -> go.Figure:
    if df.is_empty():
        return _empty("No data in the selected window", 350, template)

    by_dist = resolution_rate_by_district(df)
    fig = go.Figure(go.Bar(
        x=by_dist["police_district"], y=by_dist["resolution_rate"],
