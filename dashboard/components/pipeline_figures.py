...
def dag_timeline_bar(
    pipeline: pl.DataFrame, template: str = "terminal_amber"
) -> go.Figure:
...
def test_pass_rate_line(
    tests: pl.DataFrame, template: str = "terminal_amber"
) -> go.Figure:
    ...
    fig.update_layout(
        **tu.base_layout(template),
        ...
        height=380,
        yaxis=dict(
            title="Pass rate %", range=[0, 105], gridcolor=tu.grid_color(template)
        ),
        xaxis_title=None,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
    )
            ...
            cells=dict(
                values=[
                    ...
                    [
                        ...
                        for v in data["pass_rate_30d"].to_list()
                    ],
                ],
