    ...
    [
        ...
        dbc.Row(
            [
                dbc.Col(
                    dbc.Card(
                        dbc.CardBody(dcc.Graph(id="eng-fig-success-rate")),
                ...
                dbc.Col(
                    dbc.Card(
                        dbc.CardBody(dcc.Graph(id="eng-fig-runtime")),
                        className="shadow-sm h-100",
                    ),
                    md=6,
                ),
        ...
        dbc.Row(
            [
                dbc.Col(
                    dbc.Card(
                        dbc.CardBody(dcc.Graph(id="eng-fig-test-bar")),
                        className="shadow-sm h-100",
                    ),
                    md=7,
                ),
                dbc.Col(
                    dbc.Card(
                        dbc.CardBody(dcc.Graph(id="eng-fig-test-table")),
                        className="shadow-sm h-100",
                    ),
                    md=5,
                ),
...
def refresh(theme: str):
    ...
    if not PIPELINE_SUMMARY.is_empty():
        s = PIPELINE_SUMMARY
        success_7d = (
            round(s["success_rate_7d"].mean(), 1)
            if "success_rate_7d" in s.columns
            else None
        )
        failures_7d = (
            int(s["failures_7d"].sum()) if "failures_7d" in s.columns else None
        )
        avg_dur = (
            s["avg_duration_7d_seconds"].mean()
            if "avg_duration_7d_seconds" in s.columns
            else None
        )
        dur_chg = (
            s["duration_change_pct"].mean()
            if "duration_change_pct" in s.columns
            else None
        )
        records_7d = (
            int(s["total_records_7d"].sum())
            if "total_records_7d" in s.columns
            else None
        )
    else:
    ...
    kpi_cards = [
        dbc.Col(
            build_kpi_card("7-Day Success Rate", _fmt_pct(success_7d), None, accent),
            md=3,
        ),
        dbc.Col(
            build_kpi_card(
                "Failures (7d)",
                str(failures_7d) if failures_7d is not None else "—",
                None,
                "#ffb300" if theme == "dark" else "#C4820A",
        ...
        dbc.Col(
            build_kpi_card(
                ...
                _fmt_dur(avg_dur),
                dur_chg,
                "#ff6b35" if theme == "dark" else "#D9661F",
                higher_is_better=False,
            ),
            md=3,
        ),
        dbc.Col(
            build_kpi_card(
                ...
                _fmt_int(records_7d),
                None,
                "#4caf66" if theme == "dark" else "#16a34a",
            ),
            md=3,
        ),
    ]
...
def _build_dag_status_cards(theme: str) -> dbc.Row:
    ...
    for dag_id in _DAG_ORDER:
        ...
        if status == "healthy":
            border_style = (
                "3px solid #16a34a" if theme == "light" else "3px solid #4caf66"
            )
            badge_color = "success"
        elif status == "degraded":
            border_style = (
                "3px solid #C4820A" if theme == "light" else "3px solid #ffb300"
            )
            badge_color = "warning"
        else:
            border_style = (
                "3px solid #dc2626" if theme == "light" else "3px solid #ef5350"
            )
            badge_color = "danger"
